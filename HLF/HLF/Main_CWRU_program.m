...
addpath ('minFunc');
addpath ('libsvm');
% name='CWRU_data_freqny_highlevel'
 %% Load your data
...
for i = 1:k
    ...
    netconfig{i}.layersizes{2} = 50;
    netconfig{i}.layersizes{3} = 25;
end
...
%    % testing phase
% %        test_hlevel_data{i} = stackedAE_Out(pp_test_data{i}, finetuned_stack{i}); %Generate the high level features that
%        [res_class{i}] = classify_clsfr_OAO(test_hlevel_data{i}', Bin_clsfr{i}, l_node, r_node);
%        err_number(i) = sum(res_class{i} ~= test_gp{i}');
%        Accuracy(i) = (length(test_gp{i}) - err_number(i)) / length(test_gp{i});
    end
