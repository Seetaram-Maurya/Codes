function temp_FV = fun_featureExtraction_generalData(data,FS,FEA_choice)

% FEATURE EXTRACTION
% A) Time domain-------------------------------------------------
if FEA_choice==1 || FEA_choice==286|| FEA_choice==629
%    fprintf('Feature Extraction of Time domain features is in progress...\n');
   tic_time = tic;
   absmean=sum(abs(data))/length(data);                % Absolute Mean
   maxpeak=max(abs(data));                               % Max Peak ?? Take mean of peaks
   rms=sqrt((sum(data.^2))/length(data));              % Rms Value
   var=sum((data-mean(data)).^2)/(length(data)-1);   % Variance
   kurt=kurtosis(data);                                  % Kurtosis
   crfac=maxpeak/rms;                                      % Crest Factor
   sf=rms/abs(mean(data));                               % Shape Factor
   skew= skewness(data);                                 % Skewness
   FV_time(1:8) = [absmean maxpeak rms var kurt crfac sf skew];
   tic_time = toc(tic_time);
...
% B) Frequency domain--------------------------------------------
if FEA_choice==2 || FEA_choice==286|| FEA_choice==629
%      fprintf('Feature Extraction of Frequency domain features is in progress...\n');
     tic_freq = tic;
     powe=nextpow2(FS);
     H1=fft(data,2^powe);                % total points (power of 2)
     energy = abs(H1).^2;
     points = floor(FS/(64*2));                                % 8 bins & half points are the Mirror image 
     for i=1:64
        HH(i)=sum(energy((points*(i-1)+1):(points*i)));
     end
     FV_freq = HH/sum(abs(energy));
     tic_freq = toc(tic_freq);
...
if FEA_choice==3 || FEA_choice==286|| FEA_choice==629
%   a. continous wavlet domain  
        ...
        morc=conv(data,morl);    % convolve data signal with morlet coefficients
        clear morl t a b;
        ...
        tic_morlet = toc(tic_morlet);
        clear morc i  count temp;
end
        

    % b. Discrete Wavelet Transform(DWT)
if FEA_choice == 4 || FEA_choice==286|| FEA_choice==629
    tic_DWT = tic;    
    input=data;
...
if FEA_choice == 5 || FEA_choice==286|| FEA_choice==629
...
if FEA_choice==6 ||  FEA_choice==292|| FEA_choice==629 
...
if FEA_choice==7 ||  FEA_choice==292|| FEA_choice==629
        fprintf('Feature Extraction using WVD is in progress...\n');
        tic_WVD = tic;
        data_samp = downsample(data,5);         % downsampling of the data
...
% F) Autocorrelation----------------------------------------------
if FEA_choice==8 || FEA_choice==292 || FEA_choice==629
    fprintf('Feature Extraction using Autocorrelation is in progress...\n');
    tic_AC = tic;
    AC = autocorr(data,length(data)-1);         %autocorrelation function has been apllied 
...
if FEA_choice==9 ||  FEA_choice==292 || FEA_choice==629
        ...
        umorc=conv(data,umorl);   %convolution of data and umorl function has been done
...
if FEA_choice==10 ||  FEA_choice==292 || FEA_choice==629
        ...
        smorc=conv(data,sirl);   % convolution of transformed sine and input data
...
%I)Pseudo Wigner-Ville Distribution---------------------------------
    if FEA_choice==11 ||  FEA_choice==292 || FEA_choice==629
        tic_PWVD = tic;
        data_samp=downsample(data,5);                % downsampling of the data
...
%J) S-Transform---------------------------------
    if FEA_choice==12 || FEA_choice==292  || FEA_choice==629
...
%K) Choi-William Distribution---------------------------------
 if FEA_choice==13 || FEA_choice==101  || FEA_choice==629
        fprintf('Feature Extraction using Choi-William Distribution is in progress...\n');
        tic_CWD = tic;
        data_samp=downsample(data,10);       % downsampling of the data
...
%L) Short Time Fourier Transform---------------------------------
 if FEA_choice==14 || FEA_choice==292  || FEA_choice==629
...
%M) Born jordan Distribution---------------------------------
 if FEA_choice==15  || FEA_choice==292  || FEA_choice==629
...
% --------------------------------------------------------------
% testrun_time = [tic_time, tic_freq, tic_morlet, tic_DWT, tic_WPT, tic_DCT, tic_WVD, tic_AC, tic_UMT, tic_CS, tic_PWVD, tic_ST,tic_CWD, tic_STFT, tic_BJD];
temp_FV=[FV_time,FV_freq,FV_cwt,FV_dwt,FV_wpt];   % argument to be returned.
